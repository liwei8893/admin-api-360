...
return [
    ...
    'ttl' => \Hyperf\Support\env('JWT_TTL', 86400), // token过期时间，单位为秒
    ...
    'blacklist_prefix' => 'jwtBlacklist:', // 黑名单缓存的前缀
    ...
    'scene' => [
        ...
        'api' => [
            ...
            'sso_key' => 'id',
            'ttl' => \Hyperf\Support\env('JWT_TTL', 86400), // token过期时间，单位为秒
            'blacklist_cache_ttl' => \Hyperf\Support\env('JWT_TTL', 86400), // 黑名单缓存token时间，注意：该时间一定要设置比token过期时间要大一点，默认为100秒,最好设置跟过期时间一样
        ],
        'app' => [
            'secret' => \Hyperf\Support\env('JWT_APP_SECRET', 'api_verify'), // 非对称加密使用字符串,请使用自己加密的字符串
            'login_type' => 'sso', //  登录方式，sso为单点登录，mpop为多点登录
            'sso_key' => 'id',
            'ttl' => \Hyperf\Support\env('JWT_TTL', 86400), // token过期时间，单位为秒
            'blacklist_cache_ttl' => \Hyperf\Support\env('JWT_TTL', 86400), // 黑名单缓存token时间，注意：该时间一定要设置比token过期时间要大一点，默认为100秒,最好设置跟过期时间一样
